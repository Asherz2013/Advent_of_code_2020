...
number_valid = 0
# Part 1 - How many contain the right number of chars in the given range.
for i in range(len(database)):
    entry = database[i]
    requirements = entry.split(":")[0]
    password = entry.split(":")[1].lstrip()
    numberrange = requirements.split(" ")[0]
    requiredchar = requirements.split(" ")[1]
    minnumber = int(numberrange.split("-")[0])
    maxnumber = int(numberrange.split("-")[1])
    numcorrectchars = 0
    for char in password:
        if char == requiredchar:
            numcorrectchars += 1
    if numcorrectchars >= minnumber and numcorrectchars <= maxnumber:
        number_valid += 1
print(f"Part 1: There are {number_valid} valid passwords out of {len(database)}")

# Part 2 - "Range" is now postion and the chat must only appear in 1 of those locations
number_valid = 0
for i in range(len(database)):
    entry = database[i]
    requirements = entry.split(":")[0]
    ...
    numberrange = requirements.split(" ")[0]
    requiredchar = requirements.split(" ")[1]
    firstindex = int(numberrange.split("-")[0])-1
    secondindex = int(numberrange.split("-")[1])-1
    
    firstchar = password[firstindex]
    secondchar = password[secondindex]

    if firstchar == requiredchar and secondchar != requiredchar:
        number_valid += 1
    elif firstchar != requiredchar and secondchar == requiredchar:
        number_valid += 1

print(f"Part 2: There are {number_valid} valid passwords out of {len(database)}")